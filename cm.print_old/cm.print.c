...
void cm_print_cm_ptr(t_cm_print *x, t_symbol *s, long argc, t_atom *argv) {
//    t_symbol *temp;
    int Flag = TRUE;    //- ver qué es esto
    int ContElem = 0, n, j, i;
    char *str;
    char *num;
//    CM *tempcm;
    ...
    *num=0;
    ...
    object_post((t_object *)x, "matrix · MaxElemxPos: %i", x->cm->MaxElemxPos);
    strcat(str,"");     //- \n
    for(j = 0; j < x->cm->NroFilas; j++){
        for(i = 0; i < NEXTC * x->cm->NroCols; i++){
            if(x->cm->mat[i][j] == SPAC){
                ...
                Flag = TRUE;  ContElem++;
                if(x->cm->mat[i][j] == 10)strcat(str, "10");
                else if(x->cm->mat[i][j] == 11)strcat(str, "11");
