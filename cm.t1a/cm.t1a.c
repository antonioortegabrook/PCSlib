/*  cm_t1a object  by Oscar Pablo Di Liscia - Max version by Antonio Ortega Brook
...
typedef struct _cm_t1a {	// defines our object's internal variables for each instance in a patch
...
void cm_t1a_pcs_ptr(t_cm_t1a *x, t_symbol *s, long argc, t_atom *argv);

void cm_t1a_assist(t_cm_t1a *x, void *b, long m, long a, char *s);
void cm_t1a_free(t_cm_t1a *x);
void *cm_t1a_new();

static t_class *cm_t1a_class;		// global pointer to the object class - so max can reference the object
...
void ext_main(void *r)
{
	...
	c = class_new("cm.t1a", (method)cm_t1a_new, (method)cm_t1a_free, sizeof(t_cm_t1a), 0L, 0);
    
    class_addmethod(c, (method)cm_t1a_pcs_ptr,        MPID,          A_GIMME, 0);    // pointer to PCS
    
    class_addmethod(c, (method)cm_t1a_assist,         "assist",      A_CANT, 0);	// (optional) assistance method
...
void *cm_t1a_new() {
	t_cm_t1a *x;                                             // local variable (pointer to a t_cm_t1a data structure)

	x = (t_cm_t1a *)object_alloc(cm_t1a_class);           // create a new instance of this object
